...
if [[ -n "$PI_ENTRYPOINT" ]]; then
    tmux new-session -d -s "$SESSION_NAME" -- bash -lc 'exec "$1" -e "$2" --whatsapp-pi-online' bash "$PI_COMMAND" "$PI_ENTRYPOINT"
else
    tmux new-session -d -s "$SESSION_NAME" -- bash -lc 'exec "$1" --whatsapp-pi-online' bash "$PI_COMMAND"
fi
